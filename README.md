...
This repository is based on the original [Web Content Extraction Benchmark](https://github.com/chatnoir-eu/web-content-extraction-benchmark) by Bevendorff et al. (2023). The original repository contains code and data for the paper *"An Empirical Comparison of Web Content Extraction Algorithms"* (Bevendorff et al., 2023).

## About This Fork

This repository extends the original benchmark with the following enhancements:
...
## Extract data
In the next step, you should extract the datasets. There is a compressed tarball called ``datasets/combined.tar.xz``, which contains all datasets in a common format. Extract it into the same directory as follows (run this from within the `datasets` directory):
...
```console
wceb eval score [all|rouge|levenshtein|bleu]
```

This will calculate the ROUGE-LSum, Levenshtein, and BLEU scores for the existing extractions.
...
```console
wceb eval aggregate [all|rouge|levenshtein|bleu]
```
...
This fork includes the following modifications and additions:
...
- Repository: [https://github.com/chatnoir-eu/web-content-extraction-benchmark](https://github.com/chatnoir-eu/web-content-extraction-benchmark)
