...
def extract(html):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # First step: Extract features using Scala
        print("Running ExtractPageFeatures...")
        ...
        if process.returncode != 0:
            print("ExtractPageFeatures failed with error:")
            print("STDOUT:", stdout.decode())
            print("STDERR:", stderr.decode())
            raise RuntimeError('Web2Text ExtractPageFeatures failed.')

        # Second step: Classify using Python
        ...
        stdout, stderr = process.communicate()
        if process.returncode != 0:
            print("Classification failed with error:")
            print("STDOUT:", stdout.decode())
            print("STDERR:", stderr.decode())
            raise RuntimeError('Web2Text DOM node classification failed.')

        # Third step: Apply labels using Scala
        print("Running ApplyLabelsToPage...")
        process = subprocess.Popen(
        ...
        stdout, stderr = process.communicate()
        if process.returncode != 0:
            print("ApplyLabelsToPage failed with error:")
            print("STDOUT:", stdout.decode())
            print("STDERR:", stderr.decode())
