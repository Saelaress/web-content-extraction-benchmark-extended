...
def bleu_eval(key, model, dataset, target, pred):
    # Обработка пустых текстов
    if target.strip() == '':
        return [dict(
            hash_key=key,
            model=model,
            bleu_score=1.0 if pred.strip() == '' else 0.0,
            scorer='bleu',
            dataset=dataset
        )]
    ...
    # Сглаживание для избежания нулевых значений
    smoothing = SmoothingFunction().method1
    
    # Веса для разных n-грамм (больший вес для 1-грамм и 2-грамм)
    weights = (0.4, 0.3, 0.2, 0.1)
    
    try:
        # Для коротких текстов используем только 1-граммы и 2-граммы
        if len(target_tokens) < 4 or len(pred_tokens) < 4:
            weights = (0.6, 0.4, 0.0, 0.0)
        
        score = sentence_bleu(
            [target_tokens],  # Список референсных предложений
            pred_tokens,
            smoothing_function=smoothing,
            weights=weights
        )
    except:
        score = 0.0  # На случай ошибок вычисления
...
def _write_agg_df_to_files(df, score_name, main_score_col, out_file_base):
    ...
    for series in '_micro', '_macro':
        ...
        if score_name == 'rouge':
            ...
            out_df_reduced.columns = ['Mean BLEU', 'Median BLEU']
        else:
...
def aggregate_scores(score_name, models, datasets, complexities):
    ...
    :param score_name: score to aggregated (``"rouge"`` or ``"levenshtein"`` or ``"bleu"``)
    ...
    if score_name == 'rouge':
        ...
        score_cols = ['bleu_score']
        main_score_col = 'bleu_score'
    else:
        score_cols = ['dist']
        main_score_col = 'dist'
    ...
    if score_name == 'rouge':
        ...
        title_box = 'Median BLEU Scores'
        title_bar = 'Mean BLEU Scores'
        title_hist = 'BLEU Scores'
    else:
